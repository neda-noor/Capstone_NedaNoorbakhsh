...
#suicidedata<-read.csv(file="C:/Neda/MyProject/master.csv", header=TRUE, sep=",") 
head(suicidedata)
summary(suicidedata)
str(suicidedata)
...
ggplot(country_mean_gdp, aes(x = gdp_per_capita, y = suicide_per_100k, col = continent)) + 
  ...
  labs(title = "Correlation between GDP (per capita) and Suicides per 100k", 
       ...
       col = "Continent") 
```
# check the Linear Regression model between GDP per capita and Suicide per 100K
```{r}
...
suicidedata_n <- suicidedata %>% 
  dplyr::select(gdp_per_capita,population)

suicidedata_normal <- as.data.frame(lapply(suicidedata_n, normalize))

suicidedata_normal <- cbind(Suicide_Rate,suicidedata_normal)
suicidedata_normal <- na.omit(suicidedata_normal)
...
```{r}
set.seed(1)
index <- sample(1:nrow(suicidedata_normal), 0.65 * nrow(suicidedata_normal))
train<-suicidedata_normal[index,]
test<-suicidedata_normal[-index,]

train_target_labels<-train[,1]
Actual<-test[,1]
```
...
test <- suicidedata_normal[-rn_train,]
 
model_mlr <- lm(suicides_no   ~ . , data=train) 
summary(model_mlr)
...
There are significant positive linear relationships between number of suicides and being a "Male" , living in "Asia" and the "Population" of the country.
It seems that p-value for gdp_per_Capita is > 0.05 whih means we can not *reject* the null hypothesis that a countries GDP (per capita) has no association with it's number of suicide.
